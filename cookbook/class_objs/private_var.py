...
# 名规约来达到这个效果。第一个约定是任何以单下划线开头的名字都应该是内部实现。比如
class A:
    def __init__(self):
...
# 模块级别函数。例如，如果你看到某个模块名以单下划线开头 (比如_socket)，那它就
# 是内部实现。类似的，模块级别函数比如 sys._getframe() 在使用的时候就得加倍小心了。
# 你还可能会遇到在类定义中使用两个下划线 ( ) 开头的命名。比如：
class B:
    def __init__(self):
        self.__private = 0
...
# 属性会被分别重命名为 B__private 和 B__private method 。这时候你可能会问这样
# 重命名的目的是什么，答案就是继承——这种属性通过继承是无法被覆盖的。比如：
class C(B):
    def __init__(self):
        super().__init__()
...
# 这里，私有名称 __private 和__private method 被重命名为 C__private 和
# C__private method ，这个跟父类 B 中的名称是完全不同的
