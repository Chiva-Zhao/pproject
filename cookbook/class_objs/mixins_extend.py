...
# 可以利用它们来构造你自己的类。假设你想扩展映射对象，给它们添加日志、唯一性设置、
# 类型检查等等功能。下面是一些混入类
...
# 这个例子中，可以看到混入类跟其他已存在的类 (比如 dict、 defaultdict
# 和 OrderedDict) 结合起来使用，一个接一个。结合后就能发挥正常功效了
# 混入类在标志库中很多地方都出现过，通常都是用来像上面那样扩展某些类的功
# 能。它们也是多继承的一个主要用途。比如，当你编写网络代码时候，你会经常使用
# socketserver 模块中的 ThreadingMixIn 来给其他网络相关类增加多线程支持。
