# 减少可调用对象的参数个数
# 如 果 需 要 减 少 某 个 函 数 的 参 数 个 数， 你 可 以 使 用 functools.partial() 。
...
# 更进一步， partial() 通常被用来为调其他库函数所使用的回调函数的参数。例
...
# 参数。而 multiprocessing 对这些一无所知——它仅仅只是使用单个值来调用回调函数
...
# 讲也更加难懂。这时候使用 partial() 可以更加直观的表达你的意图 (给某些参数预先赋值)。
