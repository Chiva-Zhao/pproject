...
# 性。我们最主要关注两个类： Signature 和 Parameter 。下面是一个创建函数前面的交互例子
import inspect
...
# fun(1, 2, 3, 4) #too many positional arguments
# fun(y=2) #TypeError: missing a required argument: 'x'
# fun(1, y=2, x=3) #TypeError: multiple values for argument 'x'
...
# 己的参数检验时，代码就会笨拙混乱。这时候我们可以通过一个签名对象来简化它
...
# 用的。这样的话，在使用 inspect 模块执行内省的代码就能发现签名并将它作为调用约定
print(inspect.signature(Stock))
